...
namespace hpl {
  ...
  void iFontData::DrawImmediate(const cVector3f& avPos, const cVector2f& avSize, const cColor& aCol,
    eFontAlign aAlign, const wchar_t* fmt, ...)
  {
    wchar_t sText[256];
    va_list ap;
    ...
    while (sText[lCount] != 0)
    {
      ...
      if (pGlyph)
      {
        ...
        auto ofs = pGlyph->mpGfxObject->GetMaterial()->GetTextureOffset(eMaterialTexture_Diffuse);

        vtx[0] = cVertex(vPos + vOffset + cVector3f(0.0f, 0.0f, 0.0f), cVector2f(ofs.x, ofs.y), aCol);
        vtx[1] = cVertex(vPos + vOffset + cVector3f(vSize.x, 0.0f, 0.0f), cVector2f(ofs.x + ofs.w, ofs.y), aCol);
        vtx[2] = cVertex(vPos + vOffset + cVector3f(vSize.x, vSize.y, 0.0f), cVector2f(ofs.x + ofs.w, ofs.y + ofs.h), aCol);
        vtx[3] = cVertex(vPos + vOffset + cVector3f(0.0f, vSize.y, 0.0f), cVector2f(ofs.x, ofs.y + ofs.h), aCol);
        
        mpLowLevelGraphics->SetTexture(0, pGlyph->mpGfxObject->GetMaterial()->GetTexture(eMaterialTexture_Diffuse));
        mpLowLevelGraphics->SetBlendFunc(eBlendFunc_SrcAlpha, eBlendFunc_OneMinusSrcAlpha);
        mpLowLevelGraphics->SetBlendActive(true);
        mpLowLevelGraphics->SetCullActive(false);

        mpLowLevelGraphics->DrawQuad(vtx);
  ...
  int iFontData::DrawWordWrapWorld(cVector3f avPos, float afLength, float afFontHeight, cVector2f avSize, const cColor& aCol,
    eFontAlign aAlign, const tWString &asString, cMatrixf transform)
  {
    int rows = 0;
    ...
    mpLowLevelGraphics->SetMatrix(eMatrix_ModelView, transform);
    ...
    if (rows == 0)
    {
      DrawImmediate(avPos, avSize, aCol, aAlign, _W("%ls"), asString.c_str());
    }
    else
    {
      first_letter = 0;
      ...
      for (tUIntListIt it = RowLengthList.begin(); it != RowLengthList.end(); ++it)
      {
        DrawImmediate(cVector3f(avPos.x, avPos.y + i*afFontHeight, avPos.z), avSize, aCol, aAlign,
          _W("%ls"), asString.substr(first_letter, *it - first_letter).c_str());
        i++;
        first_letter = *it + 1;
      }
