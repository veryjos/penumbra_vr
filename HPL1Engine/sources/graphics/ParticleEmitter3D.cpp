...
namespace hpl {
	...
	void iParticleEmitter3D::UpdateGraphics(cCamera3D *apCamera,float afFrameTime, cRenderList *apRenderList, bool dontUpdateLast)
	{
		if(apCamera==NULL) return;

    if (!dontUpdateLast) {
      mfLastFrameTime = afFrameTime;
      mpLastRenderList = apRenderList;
    }
...
//				}
	...
	{
    // For VR and stereo rendering, must update the render buffer again
    // because the coordinate systems are fucked up for this particle system.
    UpdateGraphics(apCamera, mfLastFrameTime, mpLastRenderList);
