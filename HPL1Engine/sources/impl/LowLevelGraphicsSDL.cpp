...
namespace hpl {
  ...
  void cLowLevelGraphicsSDL::SetVREnabled(bool enable, float width, float height)
  {
    mbVrEnabled = enable;

    if (width != 0.0f && height != 0.0f) {
      mfVrScreenWidth = width;
      mfVrScreenHeight = height;
    }
    else {
      mfVrScreenWidth = GetScreenSize().x;
      mfVrScreenHeight = GetScreenSize().y;
    }
  }

  //-----------------------------------------------------------------------

  bool cLowLevelGraphicsSDL::GetVREnabled()
  {
    return mbVrEnabled;
  }
	...
	{
		glScissor(aRect.x, (GetScreenSize().y - aRect.y - 1)-aRect.h, aRect.w, aRect.h);
	}
							...
							const cVector2l &avSize, const cVector2l &avTexOffset)
	{
		...
		int lScreenY = (GetScreenSize().y - avSize.y) - avPos.y;
	...
	{
    if (mbVrEnabled)
      return cVector2f(mfVrScreenWidth, mfVrScreenHeight);
