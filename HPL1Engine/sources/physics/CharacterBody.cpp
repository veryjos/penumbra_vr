...
namespace hpl {
    ...
    mpCollideCallbackStepUp = hplNew(cCharacterBodyCollideStepUp, ());
		
		mpCollideCallbackGravity->mpCharBody = this;
		mpCollideCallbackPush->mpCharBody = this;
    mpCollideCallbackStepUp->mpCharBody = this;
    ...
    hplDelete(mpCollideCallbackStepUp);
	}
	...
	bool cCharacterBodyRay::OnIntersect(iPhysicsBody *pBody,cPhysicsRayParams *apParams)
	{
		...
		{
      if (mbIsPlayer && !pBody->GetCollidePlayer())
        return true;
    ...
    if (mpCharBody->vr_velocity.Length() == 0 &&
		  (mpCharBody->GetMoveSpeed(eCharDir_Forward)==0 &&
			mpCharBody->GetMoveSpeed(eCharDir_Right)==0)) return;
		
		//Check what bodies not to push.
		if(apBody->GetMass()==0 || apBody->GetMass() > mpCharBody->GetMaxPushMass()) return;
		...
		if(bPush)
		{
			...
			if(mpCharBody->GetPushIn2D())
			{
				...
				vDir.y =0; vDir.Normalise();

				apBody->AddForceAtPosition(vDir * mpCharBody->GetPushForce() * ((mpCharBody->vr_velocity.Length() != 0.0f) ? 0.2f : 1.0f), vPoint);
			}
			else
			{
  ...
  //////////////////////////////////////////////////////////////////////////
  // COLLIDE STEP UP CALLBACK
  //////////////////////////////////////////////////////////////////////////

  //-----------------------------------------------------------------------

  cCharacterBodyCollideStepUp::cCharacterBodyCollideStepUp()
  {
    mpCharBody = NULL;
  } 
  ...
  void cCharacterBodyCollideStepUp::OnCollision(iPhysicsBody *apBody, cCollideData *apCollideData)
  {
  }
  ...
  void iCharacterBody::SetCollidePlayer(bool abX)
  {
    mbCollidePlayer = abX;
    ...
    {
      if (mvExtraBodies[i]) {
        mvExtraBodies[i]->SetCollidePlayer(mbCollidePlayer);
      }
    }
  }
  ...
  void iCharacterBody::SetIsPlayer(bool abX)
  {
    mbIsPlayer = abX;
    ...
    {
      if (mvExtraBodies[i]) {
        mvExtraBodies[i]->SetIsPlayer(mbIsPlayer);
      }
    }
  }
	...
	void iCharacterBody::SetPosition(const cVector3f& avPos, bool abSmooth)
	{
		...
		if(!abSmooth)mlstCameraPos.clear();
	}
    ...
    if (vr_velocity.Length() == 0.0f) {
      for (int i = 0; i < eCharDir_LastEnum; i++)
      {
        ...
        else
        {
		  ...
		  /////////////////////////////////////////
		  //Update postion
		  if(mbClimbing)	mfCheckStepClimbCount =0;
		  else			mfCheckStepClimbCount -= afTimeStep;
    }
    ...
    if (vr_velocity.Length() == 0.0f) {
      ...
      float fMaxStep = mfMoveSpeed[eCharDir_Forward] >= 0 ?
        mfMaxPosMoveSpeed[eCharDir_Forward] :
        mfMaxNegMoveSpeed[eCharDir_Forward];
      fMaxStep *= afTimeStep;
      ...
      vPosAdd.x = vr_velocity.x;
      vPosAdd.z = vr_velocity.z;
    }
    ...
    cVector3f wantPos = mvPosition;
		...
		if(mvLastPosition.x != mvPosition.x || mvLastPosition.z != mvLastPosition.z)
		{
			mpWorld->CheckShapeWorldCollision(&vNewPos,mpBody->GetShape(), cMath::MatrixTranslate(mvPosition),mpBody,
											false, true, mpCollideCallbackPush,mbCollideCharacter, false, mbCollidePlayer, mbIsPlayer);
		}
		else
		{
			vNewPos = mvPosition;
        ...
        // Two iterations, first without epsilon, second with epsilon.
        // Prevents speeding up on slopes, but also allows for step-up on
        // edges between cylinder verts where you'd normally be too far to
        // step up.

        float epsilon = 0.05f;

        for (int i = 0; i < 2; ++i) {
          cVector3f start = wantPos + cVector3f(0, mpBody->GetShape()->GetSize().y / 2.0f, 0) + vMoveDir * (fRadius + epsilon * i);
          cVector3f end = start - cVector3f(0, mpBody->GetShape()->GetSize().y, 0);

          mpRayCallback->Clear();
          mpRayCallback->mbIsPlayer = mbIsPlayer;
    ...
    if (vr_velocity.Length() != 0.0f) {
      ...
      vr_velocity.x = vr_velocity.y = vr_velocity.z = 0.0f;
    ...
    if (mvVelolcity.x != 0 || mvVelolcity.z != 0)
    {
      ...
      bool bCollide = mpWorld->CheckShapeWorldCollision(&vNewPos, mpBody->GetShape(), cMath::MatrixTranslate(mvPosition),
        mpBody, false, true, NULL, mbCollideCharacter, false, mbCollidePlayer, mbIsPlayer);
    ...
    bCollide = mpWorld->CheckShapeWorldCollision(&vNewPos, mpBody->GetShape(),
      ...
      mpCollideCallbackGravity, mbCollideCharacter, false, mbCollidePlayer, mbIsPlayer);
    ...
    else
    {
      ...
      if (vLastVelocity.y < 0 && mvVelolcity.y > 0) mvVelolcity.y = 0;
      if (vLastVelocity.y > 0 && mvVelolcity.y < 0) mvVelolcity.y = 0;
    }
    ...
    else
    {
      if (bCollide && vLastVelocity.y <= 0 && std::abs(mvVelolcity.y) < (std::abs(vLastVelocity.y) - 0.001f))
      {
        if (mbOnGround == false)
        {
          if (mpCallback) mpCallback->OnHitGround(this, vLastVelocity);
        }
    ...
    if (mbOnGround && mvVelolcity.y<0 && mvVelolcity.y > -0.15)
    {
      mvPosition = vPosbeforeGrav;
		...
		if(pLastAttached != mpAttachedBody)	mbAttachmentJustAdded = true;
		UpdateAttachment();
	...
	{
		if(apBody == mpAttachedBody) return;

		if(mpAttachedBody) mpAttachedBody->RemoveAttachedCharacter(this);

		mpAttachedBody = apBody;
		if(mpAttachedBody) mpAttachedBody->AddAttachedCharacter(this);
	}
  ...
  kSerializeVar(mbCollidePlayer, eSerializeType_Bool)
  //kSerializeVar(mbIsPlayer, eSerializeType_Bool)
    ...
    kSaveData_SaveTo(mbCollidePlayer);
    //kSaveData_SaveTo(mbIsPlayer);
    ...
    kSaveData_LoadFrom(mbCollidePlayer);
    //kSaveData_LoadFrom(mbIsPlayer);
    ...
    SetCollidePlayer(mbCollidePlayer);
    SetIsPlayer(false);
	}
