...
namespace hpl {
    ...
    const cVector3f& GetRealPosition() {
      return mvPosition;
    }
    ...
    cVector2f Project(const cVector3f& avWorldPos, iLowLevelGraphics *apLowLevel);
		...
		float GetYaw() const{ 
      if (use_vr_view_matrix)
        return cMath::GetAngleFromPoints3D(cVector3f(0, 0, 0), vr_view_matrix.GetRotation().GetForward()*-1).y;
      else
        return mfYaw;
    }
    ...
    bool use_vr_projection_matrix;
    cMatrixf vr_projection_matrix;

    bool use_vr_view_matrix;
    cMatrixf vr_view_matrix;
