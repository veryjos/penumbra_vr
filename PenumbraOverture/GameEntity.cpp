...
eCrossHairState iGameEntity::GetPickHandState(iPhysicsBody *apBody)
{
...
eCrossHairState iGameEntity::GetCompletePickCrosshairState(iPhysicsBody *apBody) {
  auto lastState = mpInit->mpPlayer->GetCrossHairState();
  auto lastPick = mpInit->mpPlayer->GetExamineBody();

  eCrossHairState returnState = eCrossHairState_None;

  mpInit->mpPlayer->SetCrossHairState(eCrossHairState_None);

  PlayerPick();
  returnState = mpInit->mpPlayer->GetCrossHairState();

  if (returnState == eCrossHairState_None || returnState == eCrossHairState_Examine)
    returnState = GetPickCrossHairState(apBody);

  mpInit->mpPlayer->SetCrossHairState(lastState);
  mpInit->mpPlayer->SetExamineBody(lastPick);
...
eCrossHairState iGameEntity::GetPickCrossHairState(iPhysicsBody *apBody, bool noInteract)
{	
	float fDistance = GetPickedDistance();
