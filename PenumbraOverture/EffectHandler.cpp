...
void cEffect_SubTitle::Add(const tWString& asMessage, float afTime, bool abRemovePrevious, cMatrixf transform, bool aStartActive) {
  abRemovePrevious = false;

  if (transform == cMatrixf::Identity) {
    cCamera3D* camera = (cCamera3D*)mpInit->mpGame->GetScene()->GetCamera();

    // Scale down the text
    transform = cMath::MatrixMul(cMath::MatrixScale(cVector3f(0.00175f, -0.00175f, 0.00175f)), transform);
	...
	subTitle.mbActive = aStartActive;
  subTitle.mmTransform = transform;
...
void cEffect_SubTitle::Update(float afTimeStep)
{
	...
	for(; it != mlstSubTitles.end(); )
	{
      ...
      subTitle.mmTransform = cMath::MatrixMul(cMath::MatrixTranslate(cVector3f(0.0f, afTimeStep*0.05f, 0.0f)), subTitle.mmTransform);
		}
...
void cEffect_SubTitle::OnPostSceneDraw()
{
	...
	for(; it != mlstSubTitles.end(); ++it)
	{
      ...
      cMatrixf transform = subTitle.mmTransform;
      transform = cMath::MatrixMul(camera->GetViewMatrix(), transform);

			mpFont->DrawWordWrapWorld(cVector3f(0.0f, 0.0f, 0.0f) + cVector3f(1.0f, 1.0f, 0.0f),750,16,15,cColor(0,fAlpha),
				eFontAlign_Center,subTitle.msMessage, transform);
			mpFont->DrawWordWrapWorld(cVector3f(0.0f, 0.0f, 0.0f) + cVector3f(-1.0f, -1.0f, 0.0f),750,16,15,cColor(0,fAlpha),
				eFontAlign_Center,subTitle.msMessage, transform);
      mpFont->DrawWordWrapWorld(cVector3f(0.0f, 0.0f, 0.0f), 750, 16, 15, cColor(1, fAlpha),
        eFontAlign_Center, subTitle.msMessage, transform);
		}
	}
}
...
void cEffectHandler::OnDraw()
{
  ...
  // Disabled this, now handled in player OnPostSceneDraw for vr
	// mpSubTitle->OnDraw();
