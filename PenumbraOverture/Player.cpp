...
cPlayer::cPlayer(cInit *apInit)  : iUpdateable("Player")
{
  ...
  */
  ...
  mvStates[ePlayerState_InteractMode] = hplNew(cPlayerState_InteractMode_VR, (mpInit, this));
  mvStates[ePlayerState_Grab] = hplNew(cPlayerState_Grab_VR, (mpInit, this));
  mvStates[ePlayerState_WeaponMelee] = hplNew(cPlayerState_WeaponMelee_VR, (mpInit, this));
  mvStates[ePlayerState_UseItem] = hplNew(cPlayerState_UseItem_VR, (mpInit, this));
  ...
  mpPickOverlapCallback = hplNew( cPlayerPickOverlapCallback, ());

  mpExamineRayCallback = hplNew(cPlayerExamineRayCallback, ());
...
cPlayer::~cPlayer(void)
{
  ...
  hplDelete( mpPickOverlapCallback);
	...
	for(size_t i=0; i < mvCrossHairs.size(); i++)
		if(mvCrossHairs[i]) mpGfxDrawer->DestroyGfxObject(mvCrossHairs[i]);
...
{
	mfMass = afX;
	if(mpCharBody)mpCharBody->SetMass(afX);
}
...
{
	mfPower += afX;
	if(mfPower > 100) mfPower =100;
	if(mfPower < 0) mfPower =0;
}
...
{
	if(mlGroundCount<=0 && abSkipCount==false) return;
...
{
	if(afDamage <= 0) return;
	if(mpInit->mpMapHandler->IsChangingMap()) return;
	...
	if(mpInit->mbHasHaptics && aType == ePlayerDamageType_BloodSplash)
	{
		...
		else
		{
			...
			if(mpDamageForce->IsActive())	mpDamageForce->SetActive(false);
...
cPlayerPickOverlapCallback* cPlayer::GetPickOverlapCallback()
{
  return mpPickOverlapCallback;
}


iPhysicsBody* cPlayer::GetExamineBody()
{
  return mpExamineRayCallback->mpExamineBody;
}
void cPlayer::SetExamineBody(iPhysicsBody* apBody)
{
  mpExamineRayCallback->mpExamineBody = apBody;
}

//-----------------------------------------------------------------------

float cPlayer::GetExamineDist()
{
  return mpExamineRayCallback->mfPickedDist;
}

const cVector3f& cPlayer::GetExaminePos()
{
  return mpExamineRayCallback->mvPickedPos;
}

cPlayerExamineRayCallback* cPlayer::GetExamineRay()
{
  return mpExamineRayCallback;
}
...
{	
  ...
  mpVRTeleport->OnWorldLoad();

  mpVRDimmer->OnWorldLoad();
...
public:
	...
	bool OnIntersect(iPhysicsBody *pBody,cPhysicsRayParams *apParams)
	{
		...
		if(pBody->GetCollide()==false || pBody->IsCharacter()) return true;
...
void cPlayer::Update(float afTimeStep)
{
  ...
  auto plstate = GetState();

  // If the distance between the desired head pos and the player's body is too large
  // snap the player's head pos to the body pos
  cVector3f headPos2D = vr_headPos;
  headPos2D.y = 0.0f;

  cVector3f bodyPos2D = mpCharBody->GetPosition();
  bodyPos2D.y = 0.0f;

  if ((bodyPos2D - headPos2D).Length() > 0.8f) {
    vr_headPos = mpCharBody->GetPosition();
  }

  if ((plstate == ePlayerState_Normal ||
    plstate == ePlayerState_Grab ||
    plstate == ePlayerState_Throw ||
    plstate == ePlayerState_Push ||
    plstate == ePlayerState_Move ||
    plstate == ePlayerState_WeaponMelee) &&
    !mpInit->mpPlayer->GetLookAt()->IsActive() &&
    !mpInit->mpInventory->IsActive()) {

    // If the body has moved some arbitrary distance, create a footstep
    if ((vr_lastFootstepPos - bodyPos2D).Length() > 0.85f) {
      vr_lastFootstepPos = bodyPos2D;
    ...
    if (headDelta.Length() != 0.0f) {
      ...
      cVector3f bodyDelta = vr_headPos - mpCharBody->GetPosition();
      bodyDelta.y = 0.0f;
    ...
    if (vr_moveVec.Length() != 0.0f) {
      ...
      cVector3f moveDelta = mpCharBody->GetPosition() - startMove;
      moveDelta.y = 0.0f;

      if ((vr_headPos - mpCharBody->GetPosition()).Length() + 0.001f > ((vr_headPos + moveDelta) - mpCharBody->GetPosition()).Length())
        vr_headPos += moveDelta;

      vr_moveVec = cVector3f(0.0f, 0.0f, 0.0f);
    }
...
void cPlayer::OnDraw()
{
	...
	else if(mCrossHairState != eCrossHairState_None)
	{
		...
		cVector2l vSize = pImage->GetSize();
		cVector2f vPosAdd(((float)vSize.x) / 2.0f, ((float)vSize.y) / 2.0f);
		mpGfxDrawer->DrawGfxObject(mvCrossHairs[mCrossHairState],cVector3f(0,0,10)+(mvCrossHairPos - vPosAdd));
	}
	else if(mpInit->mbShowCrossHair)
	{
		...
		cVector2l vSize = pImage->GetSize();
		cVector2f vPosAdd(((float)vSize.x) / 2.0f, ((float)vSize.y) / 2.0f);
		mpGfxDrawer->DrawGfxObject(mvCrossHairs[eCrossHairState_Cross],cVector3f(0,0,10)+(vPos - vPosAdd));
	}
...
void cPlayer::LoadFromGlobal(cPlayer_GlobalSave *apSave)
{
	...
	//////////////////////////////
	// Lights
	if(apSave->mbFlashlightActive) mpFlashLight->SetActive(true);
	if(apSave->mbFlashlightDisabled) mpFlashLight->SetDisabled(true);
	if(apSave->mbGlowstickActive) mpGlowStick->SetActive(true);
