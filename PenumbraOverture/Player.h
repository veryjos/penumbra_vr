...
class cPlayerPickOverlapCallback;
class cPlayerExamineRayCallback;
...
public:
  ...
  cPlayerPickOverlapCallback* GetPickOverlapCallback();

  iPhysicsBody* GetExamineBody();
  void SetExamineBody(iPhysicsBody* apBody);
  float GetExamineDist();
  const cVector3f& GetExaminePos();
  cPlayerExamineRayCallback* GetExamineRay();
  ...
  cPlayerVRHand* GetRightHand() { return mpVRRightHand; }

  cPlayerVRHand* GetLeftHand() { return mpVRLeftHand; }

  cPlayerVRTeleport* GetVRTeleport() { return mpVRTeleport; }
  ...
  bool GetItemFlash() { return mbItemFlash; }
  ...
  cVector3f vr_headPos;
  cVector3f vr_moveVec;
  cVector3f vr_lastFootstepPos;
...
private:
  ...
  cPlayerPickOverlapCallback *mpPickOverlapCallback;
  cPlayerExamineRayCallback *mpExamineRayCallback;
  ...
  cPlayerVRHand *mpVRRightHand;
  cPlayerVRHand *mpVRLeftHand;

  cPlayerVRTeleport *mpVRTeleport;
