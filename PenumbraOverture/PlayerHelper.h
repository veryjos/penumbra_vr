...
class cPlayerExamineRayCallback : public iPhysicsRayCallback
{
public:
  cPlayerExamineRayCallback();
  ...
  iPhysicsBody *mpExamineBody;
  iPhysicsBody *mpExamineAreaBody;
...
class cPlayerPickOverlapCallback : public iPhysicsWorldCollisionCallback
{
public:
  cPlayerPickOverlapCallback();
  ...
  void CalculateResults(cVector3f center, cPlayerPickRayCallback* pickCallback);

  std::vector<iPhysicsBody*> mpPickedBodies;
  std::vector<iGameEntity*> mpPickedEntities;
  std::vector<cVector3f> mvPickedEntityCenters;
};
...
{
public:
  cPlayerVRHand(cInit *apInit, int index);
  ~cPlayerVRHand();
...
class cPlayerVRPointer
{
public:
  cPlayerVRPointer(cInit *apInit);
  ~cPlayerVRPointer();
  ...
  void Point(cVector3f start, cVector3f end);

  void OnPostSceneDraw();

private:
  int mbDraw;
  cVector3f mvStart, mvEnd;
...
class cPlayerVRTeleport : iPhysicsRayCallback
{
public:
  cPlayerVRTeleport(cInit *apInit);
  ~cPlayerVRTeleport();
...
private:
  ...
  int teleportBlackFrames;

  int mlTeleportHandIndex;

  std::vector<cVector3f> mvDrawPoints;
  cVector3f mvHitPos;

  cMeshManager* mpMeshManager;

  cMeshEntity* mpMarkerEntity;
...
class cPlayerVRDimmer
{
public:
  cPlayerVRDimmer(cInit *apInit);
  ~cPlayerVRDimmer();
  ...
  void SetDimTarget(float dimTarget, float dimSpeed);

  void OnPostSceneDraw();

private:
  float mfDimTarget, mfDimSpeed, mfDimValue;
